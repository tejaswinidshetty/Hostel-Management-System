...
if ($_SESSION['role'] == 'admin') {
    $sql = "SELECT name, year, sem, floor, roomno, capcity FROM room";
    $result = $conn->query($sql);
} else {
    $roomno = $_SESSION["roomno"];
    $sql = "SELECT name, year, sem, floor, roomno, capcity FROM room WHERE roomno = ?";
...
if ($result->num_rows > 0) {
    ...
    echo '<div class="avatar-holder">';
      ...
      echo '<h2>' . $row["name"] . '</h2>';
      echo '<br>';
      echo '<h4 title="Followers"><span class="followers">YEAR :' . $row["year"] . '</span></h4>';
     
      echo '<h5 title="Followers"><span class="followers">ROOMNO :' . $row["roomno"] . '</span></h4>';
      // echo '<br>';
      // echo '<br>';
      echo '</div>';
      ...
      echo ' <h5 title="Number of pens created by the user">SEM </h5>';;
      echo '<p>' . $row["sem"] . '</p>';
      echo '</div>';
      echo '<div class="ds projects">';
      echo ' <h5 title="Number of projects created by the user">FLOOR</h5>';;
      echo ' <p>' . $row["floor"] . '</p>';
      echo '</div>';
      echo '<div class="ds posts">';
      echo '<h5 title="Number of posts">Capacity</h5>';
      echo '<p>' . $row["capcity"] . '</p>';
      echo '</div>';
      echo '</div>';
      echo '</div>';   
        echo "</div>";
